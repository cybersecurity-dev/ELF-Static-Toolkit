...
def find_direct_syscalls_wcapstone(binary_path, elf_file):
    md = Cs(CS_ARCH_X86, CS_MODE_64)
    found_syscalls = set()
    text_section = elf_file.get_section_by_name('.text')
    if not text_section:
        print("No .text section found for Capstone analysis.")
        return found_syscalls

    code = text_section.data()
    addr = text_section.header['sh_addr']
    instructions = list(md.disasm_lite(code, addr))

    for i, (address, size, mnemonic, op_str) in enumerate(instructions):
        if mnemonic == 'syscall':
            for j in range(i - 1, max(-1, i - 10), -1):
                prev_mnemonic, prev_op_str = instructions[j][2], instructions[j][3]
                if prev_mnemonic == 'mov' and 'rax' in prev_op_str:
                    try:
                        syscall_num = int(prev_op_str.split(',')[-1].strip(), 0)
                        if syscall_num in SYSCALL_NUMBERS:
                            found_syscalls.add(SYSCALL_NUMBERS[syscall_num])
                            print(f"Capstone: Found syscall '{SYSCALL_NUMBERS[syscall_num]}' (#{syscall_num}) at 0x{address:x}")
                    except ValueError:
                        pass
                    break
        elif mnemonic == 'int' and op_str == '0x80':
            print(f"Capstone: Found int 0x80 at 0x{address:x} (32-bit syscall)")
...
def analyze_binary(binary_path):
    ...
    try:
        with open(binary_path, 'rb') as f:
            elf = ELFFile(f)
            syscalls = find_direct_syscalls_wcapstone(binary_path, elf)
            if syscalls:
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 static_syscall_extract_from_elf_wcapstone.py <path_to_binary>")
        sys.exit(1)
