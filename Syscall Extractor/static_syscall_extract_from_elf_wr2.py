...
def find_system_calls_wr2(binary_path):
    found_syscalls = set()
    try:
        r2 = r2pipe.open(binary_path)
        r2.cmd('aaa')  # Analyze all (functions, symbols, etc.)

        # Get imported symbols (e.g., from libc)
        imports = r2.cmdj('iij')  # JSON output of imports
        for imp in imports:
            imp_name = imp.get('name', '')
            if imp_name in SYSCALLS:
                found_syscalls.add(imp_name)
                print(f"Radare2: Found imported syscall '{imp_name}'")

        # Disassemble and search for syscalls (basic approach)
        r2.cmd('s sym.main || s entry0')  # Seek to main or entry point
        disasm = r2.cmdj('pdj 1000')  # Disassemble 1000 instructions
        for inst in disasm:
            if inst.get('type') == 'syscall':
                syscall_num = inst.get('syscall', -1)
                if syscall_num in SYSCALL_NUMBERS:
                    found_syscalls.add(SYSCALL_NUMBERS[syscall_num])
                    print(f"Radare2: Found direct syscall '{SYSCALL_NUMBERS[syscall_num]}' (#{syscall_num}) at 0x{inst['offset']:x}")
            elif 'opcode' in inst and 'int 0x80' in inst['opcode']:
                print(f"Radare2: Found int 0x80 at 0x{inst['offset']:x} (32-bit syscall)")
...
def analyze_binary(binary_path):
    ...
    try:
        with open(binary_path, 'rb') as f:
            ...
            # Combine results
            syscalls = find_system_calls_wr2(binary_path)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 static_syscall_extract_from_elf_wr2.py <path_to_binary>")
        sys.exit(1)
